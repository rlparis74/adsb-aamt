...
if [ -f /boot/adsb-config.txt ]; then
    UUID_FILE="/boot/adsbaamt-uuid"
else
    mkdir -p /usr/local/share/adsbaamt
    UUID_FILE="/usr/local/share/adsbaamt/adsbaamt-uuid"
    # move old file position
    if [ -f /boot/adsbaamt-uuid ]; then
        mv -f /boot/adsbaamt-uuid $UUID_FILE
    fi
fi
